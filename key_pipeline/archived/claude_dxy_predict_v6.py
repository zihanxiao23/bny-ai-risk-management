...
#   Each example shows: headline, content preview, event, content_type,
#   true_criticality, true_direction (when set), pct_15m outcome, AND
#   the point-in-time macro regime snapshot at publication -- so the model
...
# These are the key macro variables included in each GT example's snapshot.
# Mirrors the most informative fields from build_macro_context(), keeping
# the per-example block concise (not every macro col -- just the decisive ones).
_POOL_B_MACRO_FIELDS = [
    ...
    ("cftc_net_usd_zscore",   "CFTC USD z-score",   ""),
    ("cpi_yoy",               "CPI YoY",             "%"),
...
def build_tier_few_shot_block(gt_df: "pd.DataFrame", tier: int) -> str:
    ...
    This lets the model learn "given this macro regime AND this event type,
    the 15-min DXY reaction was high/not high / up/down" -- grounding
    criticality assessment in observed outcomes rather than tier defaults alone.
    ...
    if len(tier_rows) > cap:
        ...
        tier_rows = tier_rows.sort_values(
            ["true_criticality", "gt_event_number"]  # group high examples first
        ).reset_index(drop=True)
...
def agent_2_criticality(client, title, date, content, event_number, content_type,
    ...
    tier_few_shot_block:  Pool B -- GT examples from the same tier (new).
...
def analyze_article_dxy(client, title, date, content,
    ...
    Agent 2 (Criticality) -- skipped when event_number == 0 (Irrelevant);
                             ...
                             receives Pool B tier few-shot block when gt_df provided (new)
    ...
    # --- Build Pool B tier few-shot block for Agent 2 (new) ---
    ...
    if verbose and tier_few_shot_block:
        tier_rows_count = tier_few_shot_block.count("-- Tier Example")
...
def process_csv_to_csv(
    ...
    Two-pool few-shot architecture (when ground_truth_csv_path is provided):
      ...
      Pool B -- Agent 2 (Criticality/Direction):
        ...
        are injected into Agent 2's user prompt at call time. This gives the
        model real examples of how criticality and direction behave within the
        tier, anchoring predictions to observed outcomes rather than abstract
        tier defaults. No caching (per-article, varies by tier).
    ...
    if gt_df is not None:
        ...
        print(f"   Few-shot Pool B (A2)   : tier-matched GT ({len(gt_df)} rows, {label_note})")
    else:
        print(f"   Few-shot               : No")
...
if __name__ == "__main__":
    ...
    output_df = process_csv_to_csv(
        input_csv_path="data/aug_mar_input_cln.csv",
        output_csv_path="data/results_aug_mar.csv",
        ground_truth_csv_path="data/gt_ext_ft_macros_labeled.csv",   # GT with true_criticality + true_direction
