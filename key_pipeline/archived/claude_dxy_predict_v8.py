...
De-escalation events can move DXY as much as escalation events.
Mark HIGH for Tier 3 trade articles when article_timing = 'unscheduled'
(no fixed release window) AND any of the following:
  • Tariffs confirmed to take effect materially below the threatened
    level (e.g., 10% implemented vs 15% threatened) with an active
    market reaction described in the article text
  • A major court ruling invalidates or blocks a substantial tariff
    regime affecting DXY-basket economies, published on the day of
    the ruling with concrete market reaction described
  • A bilateral deal is announced that resolves a tariff standoff
    involving DXY-basket economies (EUR, JPY, GBP, CAD), with
    confirmed rate reductions and a binding implementation date

Do NOT mark high for de-escalation articles when:
...
_RELEASE_WINDOWS = {
    ...
    "data_0830": {"days": list(range(7)), "release_hour": 8, "release_min": 30,
                  "live_start": (8, 30), "live_end": (11, 0), "stale_hour": 17},
    # PMI/ISM: released 10:00 ET
    "data_1000": {"days": list(range(7)), "release_hour": 10, "release_min": 0,
                  "live_start": (10, 0), "live_end": (12, 0), "stale_hour": 17},
}
...
def classify_article_timing(article_published_utc: str, event_number: int) -> str:
    """
    ...
    if hm < window["live_start"]:
        ...
        return 'live_window'
    elif h < window["stale_hour"]:
        return 'same_day_post'
    else:
        # After stale_hour on same day → treat as stale
        return 'same_day_post'
...
def analyze_article_dxy(client, title, date, content,
    ...
    article_timing = classify_article_timing(date, event_number)
...
def process_csv_to_csv(
    ...
    for idx, row in df.iterrows():
        ...
        article_timing = classify_article_timing(date, event_num) if event_num != IRRELEVANT_EVENT_NUMBER else None
...
if __name__ == "__main__":
    ...
    output_df = process_csv_to_csv(
        ...
        output_csv_path="data/results_sept_oct_rel.csv",
