...
# ==============================================================================
# CLIENT SETUP
# Uses ANTHROPIC_API_KEY stored in Colab Secrets (🔑 sidebar → Secrets)
# ==============================================================================
...
def call_claude(client, prompt: str) -> dict:
    """
    Send a prompt to Claude Haiku and return parsed JSON.
    Claude doesn't have a native json_object response mode via this SDK,
    so we instruct it in the prompt and parse the response ourselves.
    """
...
def process_csv_to_csv(
    ...
    for idx, row in df.iterrows():
        ...
        # Rate limiting — Haiku's default tier: 50 RPM → ~1.2 s between requests is safe.
        # With 3 agents per article that's 3 calls, so 3 * 1.2 = 3.6 s minimum per row.
        # We add a small buffer between agents inside call_claude; here we add inter-row pause.
...
if __name__ == "__main__":
    ...
    output_df = process_csv_to_csv(
        ...
        output_csv_path="data/dxy_results/test.csv",
