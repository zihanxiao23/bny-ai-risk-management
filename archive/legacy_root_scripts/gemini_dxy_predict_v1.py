...
# Uses research-backed tier + event/opinion flag + article text.
...
def agent_3_criticality_assessor(client, title, date, content, event_info):
    ...
    Criticality logic:
      - Tier 1 event + is factual event + concrete detail  → high / critical
      - Tier 2 event + is factual event + concrete detail  → medium / critical
      - Tier 1 but vague/no detail                         → medium / critical
      - Opinion/analysis regardless of tier                → low / not critical
      - Tier 3 or 4 regardless of content                  → low / not critical
      - Recap of already-priced-in event                   → low / not critical
...
Assess whether this article is CRITICAL — meaning an FX analyst or portfolio manager
should act on or immediately investigate this news.

CRITICALITY RULES:

is_critical=true, criticality_level="high":
  - is_event_vs_opinion is TRUE
  - AND tier is 1 (Fed decisions, CPI/PCE, NFP, GDP, rate differentials)
  - AND article contains concrete, actionable detail

is_critical=true, criticality_level="medium":
  - is_event_vs_opinion is TRUE AND tier is 2 (risk events, retail sales, fiscal)
  - OR: is_event_vs_opinion is TRUE AND tier is 1 but article is vague/lacking detail

is_critical=false, criticality_level="low":
  - is_event_vs_opinion is FALSE (opinion, analysis, forecast, commentary)
  - OR tier is 3 or 4 regardless of content type
  - OR article recaps an event already fully priced in by markets

Also extract:
  data_point_referenced: true if article states a specific numeric value
    (e.g. "CPI at 3.2%", "NFP added 180k jobs", "Fed cut 25bps", "tariffs raised to 25%")
  reported_value: the specific value mentioned (e.g. "3.2%", "180k", "25bps cut", "25%"),
    or null if no specific value is found.
    Purpose: analyst reference for verification against consensus — do NOT assess beat/miss.
...
if __name__ == "__main__":
    ...
    output_df = process_csv_to_csv(
        ...
        output_csv_path="data/dxy_training/gemeni_test_ind1.csv",
